...
pub async fn validate_dialog(context: &Context, msg: &Message, background: &String, character: &String, text: &String) -> Result<(), String> {
    ...
    unsafe {
        ...
        if !backgrounds.contains(background) {
            msg.channel_id
                .say(&context.http, format!("Sorry, but I couldn't find `{}` as a location\nAvailable backgrounds are: `{}`.", background, &PERSISTENCE_STORAGE.get_instance().await.background_strings)).await.unwrap();
            return Err("Background not found.".to_string());
        }
        if !characters.contains(character) {
            msg.channel_id
                .say(&context.http, format!("Sorry, but I don't think that `{}` is a character in Camp Buddy\nAvailable characters are: `{}`.", character, &PERSISTENCE_STORAGE.get_instance().await.character_strings)).await.unwrap();
            return Err("Character not found.".to_string());
        }
    }
    if text.len() == 0 {
        msg.channel_id
            .say(&context.http, "At least give me something to send, you dumbass.").await.unwrap();
        return Err("Message not found.".to_string());
    }
    if JAPANESE_REGEX.is_match(text) && text.len() > 78 || text.len() > 180 {
        msg.channel_id
            .say(&context.http, "Sorry, the message limit is 180 characters for latin characters, 78 characters for Japanese <:TaigaAck2:700006264507465778>").await.unwrap();
        return Err("Message too long.".to_string());
    }
    if EMOJI_REGEX.is_match(text) || EMOTE_MENTIONS_REGEX.is_match(text) || NON_ASCII_AND_JAPANESE_REGEX.is_match(text) {
        msg.channel_id
            .say(&context.http, "I don't do emotes, mentions, non-latin and non-Japanese characters").await.unwrap();
