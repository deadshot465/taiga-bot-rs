...
async fn show_route(
    ...
    msg: &Message,
    user_id: &str,
    persistence: &RwLockReadGuard<'_, PersistenceStorage>,
) -> CommandResult {
    let user_records = persistence.user_records.as_ref().unwrap();
    let route_record = &user_records[user_id].route;
    let mut character_names: Vec<&str> = route_record.iter().map(|r| r.0.as_str()).collect();
        ...
        .send_message(&context.http, |m| {
            m.embed(|e| {
                ...
                for name in character_names.iter() {
                    ...
                    let mut endings: Vec<&str> =
                        character_record.iter().map(|r| r.0.as_str()).collect();
...
async fn show_valentine(
    ...
    let mut character_names: Vec<_> = valentine_record
        .iter()
...
async fn show_all(
    ...
    let route_record = &user_records[user_id].route;
    let valentine_record = &user_records[user_id].valentine;
    let mut route_names: Vec<&str> = route_record.iter().map(|r| r.0.as_str()).collect();
    let mut valentine_names: Vec<_> = valentine_record
        .iter()
        .map(|r| (r.0.as_str(), *r.1))
        .collect::<Vec<(&str, u16)>>();
...
async fn reset(
    ...
    msg: &Message,
    mut args: Args,
    persistence: &mut RwLockWriteGuard<'_, PersistenceStorage>,
) -> CommandResult {
    let stat_name = args.single::<String>();
    let user_id = msg.author.id.0.to_string();
    let user_records = persistence.user_records.as_mut().unwrap();
