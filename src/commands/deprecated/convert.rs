...
const VALID_UNITS: [&str; 10] = ["km", "m", "cm", "in", "ft", "mi", "au", "c", "f", "k"];

const VALID_LENGTHS: [&str; 7] = ["km", "m", "cm", "in", "ft", "mi", "au"];

const VALID_TEMPERATURES: [&str; 3] = ["c", "f", "k"];
...
pub async fn cvt(context: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if raw_target_unit.is_err() || raw_source_string.is_err() {
        let temps: String = VALID_TEMPERATURES
            .iter()
            .map(|f| f.to_string() + ", ")
            .collect();
        let lengths: String = VALID_LENGTHS.iter().map(|f| f.to_string() + ", ").collect();
    ...
    if !VALID_UNITS.contains(&target_unit.as_str()) {
        let units: String = VALID_UNITS.iter().map(|f| f.to_string() + ", ").collect();

        let error_msg =
            interface_string.errors["invalid_unit"].replace("{units}", &units[..units.len() - 2]);
    ...
    if !cvt_regex.is_match(&source_string) {
        let error_msg = interface_string.errors["wrong_pattern"]
            .as_str()
    ...
    let mut amount = cvt_regex
        .captures(&source_string)
        .unwrap()
        .get(1)
        .unwrap()
        .as_str()
    ...
    if (VALID_LENGTHS.contains(&target_unit.as_str()) && VALID_TEMPERATURES.contains(&source_unit))
        || (VALID_TEMPERATURES.contains(&target_unit.as_str())
            && VALID_LENGTHS.contains(&source_unit))
    {
    ...
    let result_message = interface_string
        .result
