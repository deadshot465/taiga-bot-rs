...
lazy_static! {
    static ref SPECIAL_DIALOG_REGEX: Regex =
        Regex::new(r"([a-z]+)\s{1}(\w+)\s{1}(\d{1})\s{1}([\w]+)\s{1}([\w]+)\s{1}(.*)").unwrap();
    static ref DIALOG_REGEX: Regex =
        Regex::new(r"([a-z]+)\s{1}(\w+)\s{1}([a-z0-9]+)\s{1}(.*)").unwrap();
...
pub async fn comic(context: &Context, msg: &Message) -> CommandResult {
    ...
    if msg.attachments.is_empty() {
        msg.channel_id
            .say(http, "The command has to be called with an attachment.")
            .await?;
        return Ok(());
    }
    ...
    msg.channel_id
        .say(http, format!("Your file name is: {}", &attachment.filename))
        .await?;
    ...
    if strings.len() > 5 {
        msg.channel_id
            .say(http, "The maximum number of images allowed is 5.")
            .await?;
        return Ok(());
    }
    ...
    for s in strings.iter() {
        if !special_regex.is_match(*s) && !dialog_regex.is_match(*s) {
            msg.channel_id
                .say(http, "One of the commands is incorrect.")
                .await?;
            return Ok(());
        }
        ...
        if special_regex.is_match(*s) {
            ...
            if !available_specializations.contains_key(specialization) {
                msg.channel_id
                    .say(
                        http,
                        "One of the commands contains nonexistent specialization.",
                    )
                    .await?;
                return Ok(());
            }
                ...
                .contains(&background.to_string()))
            {
                msg.channel_id
                    .say(
                        http,
                        "One of the commands contains a background that is not available.",
                    )
                    .await?;
                return Ok(());
            }
            ...
            if !character_available_options
                .poses
                .contains_key(&pose.to_string())
            {
                msg.channel_id
                    .say(http, "One of the commands contains an invalid pose.")
                    .await?;
                return Ok(());
            }
            ...
            if !character_available_options.poses[pose]
                .clothes
                .contains(&cloth.to_string())
            {
                msg.channel_id
                    .say(http, "One of the commands contains an invalid cloth.")
                    .await?;
                return Ok(());
            }
            ...
            {
                msg.channel_id
                    .say(http, "One of the commands contains an invalid face.")
                    .await?;
                return Ok(());
            }
            ...
            if !text_validation.0 {
                match &text_validation.1 {
                    TextError::NoMessage => {
                        msg.channel_id
                            .say(http, "One of the commands doesn't have any content.")
                            .await?;
                        return Ok(());
                    }
            ...
            let request_data = SpecializedDialog {
                ...
                is_hidden_character: specialization == "mhirosay",
                ...
                .contains(&character.to_string()))
            {
                msg.channel_id
                    .say(
                        http,
                        "One of the dialog commands contains an invalid character.",
                    )
                    .await?;
                return Ok(());
            }
