...
pub async fn time(context: &Context, msg: &Message, args: Args) -> CommandResult {
    ...
    if search_result.is_none() {
        timezone_name = search_from_google(&raw_city_name, &client)
            .await
            .unwrap_or_default();
    } else {
    ...
    if timezone_name.is_empty() {
        msg.channel_id
            .say(&context.http, interface_string.errors["no_result"].as_str())
            .await?;
        return Ok(());
    }

    let response = client
        .get(format!("http://worldtimeapi.org/api/timezone/{}", &timezone_name).as_str())
        .send()
        .await?;
    ...
    if let Ok(t) = time {
        ...
        msg.channel_id
            .say(
                &context.http,
                format!("Parsing time failed: {:?}", err).as_str(),
            )
            .await?;
    }
    drop(interface_lock);
...
async fn search_from_google(city_name: &str, client: &reqwest::Client) -> Option<String> {
    ...
    let geocode: GeocodeResponse = response
        .json()
        .await
        .expect("Failed to translate response to JSON.");
    ...
    let time_zone_data: TimezoneResponse = response
        .json()
        .await
        .expect("Failed to translate response to JSON.");
