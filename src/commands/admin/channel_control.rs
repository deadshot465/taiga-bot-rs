...
pub async fn allow(context: &Context, msg: &Message) -> CommandResult {
    ...
    let channels = regex.captures_iter(&msg.content)
...
pub async fn purge(context: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let channels: HashMap<ChannelId, GuildChannel> = msg.guild_id.unwrap().channels(&context.http).await?;
    let channel = channels.get(&msg.channel_id).unwrap();
    let messages: Vec<Message> = channel.messages(&context.http, |f| f
        .limit(amount)
        .before(&msg.id))
        .await?;
    let notice_msg = msg.channel_id.say(&context.http, format!("The last {} messages in this channel will be deleted.", &amount))
        .await?;
    tokio::time::delay_for(Duration::from_secs(5)).await;
    msg.channel_id.delete_messages(&context.http, messages).await?;
